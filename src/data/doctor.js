export const doctor = {
  id: 'dr-varsha-koti',
  name: 'Dr. Varsha R. Koti',
  qualifications: 'MBBS, MD (DVL), DNB, FRGUHS - Dermatosurgery',
  title: 'Consultant Dermatologist & Cosmetologist',
  clinic: 'Prashali Skin Sciences',
  bio: `Dr. Varsha R. Koti is a board certified Consultant Dermatologist with over 5+ years of experience specializing in clinical dermatology, dermatosurgery, lasers and aesthetic dermatology. She is dedicated to providing evidence-based, personalized care for every patient, combining medical expertise with cutting-edge technology to deliver natural-looking results.`,
  experience: '5+ Years',
  specializations: [
    'Clinical Dermatology',
    'Dermatosurgery',
    'Laser Treatments',
    'Aesthetic Dermatology',
    'Hair Restoration',
    'Cosmetic Dermatology',
  ],
  education: [
    'MBBS - Bangalore Medical College',
    'MD (DVL) - Dermatology, Venereology & Leprosy',
    'DNB - National Board of Examinations',
    'FRGUHS - Dermatosurgery',
  ],
  achievements: [
    'Published research in International Dermatology Journals',
    'Certified Aesthetic Medicine Practitioner',
    'Advanced Training in Laser & Cosmetic Dermatology',
    'Member of Indian Association of Dermatologists (IADVL)',
  ],
  image: '/images/doctorimage1.jpg',
  consultationFee: '₹500',
};
